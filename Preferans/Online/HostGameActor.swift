...
/// most phases that equals the seat whose turn it is, but in open single-whist
/// greedy play the lone whister plays the passer's dummy hand, so `decider`
/// names the whister.
...
public actor HostGameActor {
    ...
    public func applyClientAction(_ envelope: ClientActionEnvelope, sender: PlayerID?) throws -> HostUpdate {
        ...
        if let actionActor = envelope.action.actor {
            ...
            // either that seat itself or — in open single-whist greedy play —
